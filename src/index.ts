...
export function compile(input: string): string {
  ...
  const singletonInstancesByObject = new Map<string, Set<string>>();

  // Preprocess generic singleton references: convert &Name<Type> to &__singleton_Name_Type_
  code = code.replace(/&\s*(mut\s+)?([a-zA-Z_]\w*)\s*<([^<>]+)>/g, (_match, mutPart, name, typeArg) => {
    const instanceVarName = '__singleton_' + name + '_' + String(typeArg).replace(/\s+/g, '') + '_';
    const instances = singletonInstancesByObject.get(name) ?? new Set<string>();
    instances.add(instanceVarName);
    singletonInstancesByObject.set(name, instances);
    return '&' + (mutPart ? 'mut ' : '') + instanceVarName;
  });
  ...
  const buildSingletonInstanceDeclarations = (objectName: string, context: CompileContext): string => {
    const singletonInstances = singletonInstancesByObject.get(objectName);
    if (!singletonInstances) {
      return '';
    }
    let result = '';
    for (const instanceVarName of singletonInstances) {
      result += 'const ' + instanceVarName + ' = Object.create(' + objectName + '); ';
      context.definedVars.add(instanceVarName);
    }
    return result;
  };

  const buildObjectDeclarationSnippet = (declaration: string, context: CompileContext): string => {
    const objectDecl = parseObjectDeclaration(declaration);
    if (!objectDecl) {
      throw new Error('invalid object declaration');
    }
    return buildObjectDeclaration(objectDecl, context) + ' ' + buildSingletonInstanceDeclarations(objectDecl.name, context);
  };
  ...
  for (let i = 0; i < stmts.length - 1; i++) {
    ...
    while (true) {
      ...
      jsCode += buildObjectDeclarationSnippet(leadingObject.declaration, context);
  ...
  if (stmts.length > 0) {
    ...
    while (true) {
      ...
      jsCode += buildObjectDeclarationSnippet(finalObjectSplit.declaration, context);
