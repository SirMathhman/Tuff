...
import { isFunctionType, setFunctionRef } from "./functions";
...
export function handleVarDecl(
  ...
  if (semiIndex === -1) {
    ...
    const afterEq = s.slice(eqIndex + 1).trim(), trimLenDiff = s.slice(eqIndex + 1).length - afterEq.length;

    if (afterEq.startsWith("match") || afterEq.startsWith("loop")) {
      let exprBraceDepth = 0, exprParenDepth = 0, exprBraceCloseIdx = -1;
  ...
  for (let i = 0; i < declStr.length; i++) {
    if (declStr[i] === "=" && (i + 1 >= declStr.length || declStr[i + 1] !== ">") && (i === 0 || declStr[i - 1] !== "=")) {
      eqIndex = i;
      break;
    }
  }

  let varName: string, varValue: number = 0, vType = 0;
  if (eqIndex === -1) {
    // No assignment - just declaration with type
    const varPart = declStr.slice(4 + (isMut ? 4 : 0)).trim(), colonIndex = varPart.indexOf(":");
    ...
    if (vType === 0 && typeMap.has("__alias__" + typeStr)) vType = typeMap.get("__alias__" + typeStr) || 0;
    if (vType === 0 && typeMap.has("__union__" + typeStr)) return undefined;
  } else {
    // Has assignment
    const beforeEq = declStr.slice(4 + (isMut ? 4 : 0), eqIndex).trim();
    let colonIndex = -1, parenDepth = 0;
    for (let i = 0; i < beforeEq.length; i++) {
      const ch = beforeEq[i];
      if (ch === "(") parenDepth++;
      else if (ch === ")") parenDepth--;
      else if (ch === ":" && parenDepth === 0) { colonIndex = i; break; }
    }
    varName = colonIndex !== -1 ? beforeEq.slice(0, colonIndex).trim() : beforeEq;
    ...
    if (!isFunctionTypeAnnotation) {
      varValue = interpreter(exprStr, scope, typeMap, mutMap, uninitializedSet, unmutUninitializedSet);
    }
    vType = extractTypedInfo(exprStr).typeSize || (scope.has(exprStr) ? typeMap.get(exprStr) || 0 : 0);
    if (colonIndex !== -1) {
      const typeStr = beforeEq.slice(colonIndex + 1).trim();
      if (isFunctionType(typeStr)) {
        ...
        if (dType === 0 && typeMap.has("__alias__" + typeStr)) dType = typeMap.get("__alias__" + typeStr) || 0;
        if (dType === 0 && typeMap.has("__union__" + typeStr)) {
          if (vType === 0) {
            ...
            if (typeStart >= 0 && typeStart < exprStr.length) {
              const typeSuffix = exprStr.slice(typeStart), firstChar = typeSuffix[0];
              if (firstChar === "I" || firstChar === "U") vType = extractTypeSize(typeSuffix);
            }
          }
          dType = -1;
        }
        if (dType > 0) {
          if (vType > 0 && vType > dType) throw new Error(`bad type: ${vType} to U${dType}`);
          vType = dType;
        } else if (dType === -1) { /* Union type */ }
      }
    }
  }
