...
describe("selfhost analyzer (generic function values)", () => {
  test("rejects assigning unspecialized generic function to a value", async () => {
    const outDir = resolve(
      ...
      `generic-fn-value-${Date.now()}-${Math.random().toString(16).slice(2)}`
    );

    const { stage2Dir, tuffc2 } = await buildStage2Compiler(outDir);

    const badIn = resolve(stage2Dir, "bad_generic_fn_value.tuff");
    const badOut = resolve(stage2Dir, "bad_generic_fn_value.mjs");
  ...
  test("rejects extracting unspecialized generic method to a value", async () => {
    ...
    const badIn = resolve(stage2Dir, "bad_generic_method_value.tuff");
    const badOut = resolve(stage2Dir, "bad_generic_method_value.mjs");
