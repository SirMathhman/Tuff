...
interface StructValueLike {
  fields: Map<string, BindingLike>;
}
interface MaybeStructBinding {
  struct?: StructValueLike;
}
...
export function handleScannedIdent(
  ...
  if (next < src.length && src[next] === ".") {
    const memberRes = handleMemberAccess(name, next, src, envLocal, parentEnvLocal);
    if (!memberRes.ok) return memberRes as Result<ScannedResult, string>;
    return { ok: true, value: memberRes.value };
  }
