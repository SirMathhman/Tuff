...
test("returns 1 for the numeric literal 1", () => {
  expect(evaluate("1")).toEqual({ ok: true, value: 1 });
});

test("returns 3 for the expression 1 + 2", () => {
  expect(evaluate("1 + 2")).toEqual({ ok: true, value: 3 });
});

test("returns 6 for the expression 1 + 2 + 3", () => {
  expect(evaluate("1 + 2 + 3")).toEqual({ ok: true, value: 6 });
});

test("returns 1 for the expression 2 + 3 - 4", () => {
  expect(evaluate("2 + 3 - 4")).toEqual({ ok: true, value: 1 });
});

test("returns 10 for the expression 2 * 3 + 4", () => {
  expect(evaluate("2 * 3 + 4")).toEqual({ ok: true, value: 10 });
});

test("returns 14 for the expression 2 + 3 * 4", () => {
  expect(evaluate("2 + 3 * 4")).toEqual({ ok: true, value: 14 });
});

test("returns 20 for the expression (2 + 3) * 4", () => {
...
test("returns 20 for the expression { let x = 2 + 3; x } * 4", () => {
  expect(evaluate("{ let x = 2 + 3; x } * 4")).toEqual({ ok: true, value: 20 });
});

test("returns 20 for the expression let y = { let x = 2 + 3; x } * 4; y", () => {
  expect(evaluate("let y = { let x = 2 + 3; x } * 4; y")).toEqual({ ok: true, value: 20 });
});

test("returns 0 for a let binding with no body", () => {
  expect(evaluate("let x = 100;")).toEqual({ ok: true, value: 0 });
});

test("returns a structured error for unsupported expressions", () => {
...
test("reports the source position of an unexpected character", () => {
  const result = evaluate("1 + $");
...
test("keeps error positions consistent with the input when leading whitespace is trimmed", () => {
  const result = evaluate("  1 + $");
...
test("returns a structured error for an empty brace initializer", () => {
...
test("returns a structured error for a brace initializer whose let has no body", () => {
...
test("returns a structured error for undefined variables", () => {
