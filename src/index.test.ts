...
test('evaluate("let x : Bool = true; x") => 1', () => {
  expect(evaluate("let x : Bool = true; x")).toEqual({ ok: true, value: 1 });
});

test('evaluate("let x : Int = 1; x") => 1', () => {
  expect(evaluate("let x : Int = 1; x")).toEqual({ ok: true, value: 1 });
});

test('evaluate("fn get() => 100; get()") => 100', () => {
...
test('evaluate("let y = if (true) { let mut x = 0; x = false; x } else false; y") => type-mismatch error', () => {
  expect(
    evaluate("let y = if (true) { let mut x = 0; x = false; x } else false; y"),
  ).toEqual({
    ok: false,
    error: {
      kind: "type-mismatch",
      input: "let y = if (true) { let mut x = 0; x = false; x } else false; y",
      name: "x",
    },
  });
});
