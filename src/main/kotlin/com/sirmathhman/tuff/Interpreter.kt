...
fun interpret(input: String): Result<Int, String> {
	...
	val (numbers, ops) = when (parsed) {
		is Result.Err -> return parsed
...
private fun parseTokens(rawTokens: List<String>): Result<Pair<List<Int>, List<String>>, String> {
	...
	val rpn = when (rpnResult) {
		is Result.Err -> return Result.Err(rpnResult.error)
		is Result.Ok -> rpnResult.value
	}

	val evalResult = evalRpn(rpn)
	return when (evalResult) {
		is Result.Err -> evalResult
		is Result.Ok -> Result.Ok(Pair(listOf(evalResult.value), emptyList()))
	}
}
