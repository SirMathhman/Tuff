...
final class IdentifierResolver {
	...
	static Operand parseIdentifierLookup(Parser parser) {
		...
		if (parser.getIndex() < n && parser.charAt(parser.getIndex()) == '{') {
			return parseStructConstruction(parser, name);
		}
		...
		if (parser.getIndex() + 1 < n && parser.charAt(parser.getIndex()) == ':'
			...
			return parseModuleAccess(parser, name);
		}
		...
		parser.skipWhitespace();
		if (parser.getIndex() < n && parser.charAt(parser.getIndex()) == '[') {
			return parseIndexing(parser, name);
		}
	...
	private static Operand parseStructConstruction(Parser parser, String name) {
		parser.consumeChar();
	...
	private static Operand parseModuleAccess(Parser parser, String name) {
	...
	private static Operand parseMemberAccess(Parser parser, String name) {
		parser.consumeChar();
		parser.skipWhitespace();
		java.util.regex.Matcher fm = java.util.regex.Pattern.compile("^[A-Za-z_]\\w*")
				.matcher(parser.remainingInput());
	...
	private static Operand parseIndexing(Parser parser, String name) {
		parser.consumeChar();
