...
public final class FunctionCallSubstituter {
	...
	public static Result<String, CompileError> substituteAllFunctionCalls(String expr,
		...
		while (matcher.find()) {
			...
			var functionCall = expr.substring(start, closeParen + 1);

			var substitution = FunctionHandler.parseFunctionCall(functionCall,
																													 functionRegistry);
			if (substitution instanceof Result.Err<String, CompileError> err) {
				return Result.err(err.error());
			}
			var substituted = ((Result.Ok<String, CompileError>) substitution).value();
