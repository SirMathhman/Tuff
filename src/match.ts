...
export function handleMatch(
  ...
  const restStr = afterMatch.slice(parenCloseIdx + 1).trim();
  ...
  for (const caseStr of cases) {
    if (!caseStr) continue;
    if (!caseStr.startsWith("case ")) continue;
    ...
    if (pattern === "_" || Number(pattern) === matchValue) {
      const caseResult = interpreter(result, scope, typeMap, mutMap);

      // Calculate the position after the closing brace
      // We need to find where the match expression ends in the original trimmed string
