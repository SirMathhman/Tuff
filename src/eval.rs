...
impl TypeEnv {
    ...
    fn set(&mut self, name: &str, ty: Type, span: Span) -> Result<(), crate::TuffError> {
        for scope in self.scopes.iter_mut().rev() {
            if let Some((current, mutable)) = scope.get_mut(name) {
                ...
                if !Self::types_compatible(*current, ty) {
    ...
    /// Whether `ty` may be assigned to a binding currently of type `current`.
    fn types_compatible(current: Type, ty: Type) -> bool {
        current == ty
    }

    /// The name of a type, for error messages.
    ...
    match expr {
        ...
        Expr::If(cond, then, otherwise, span) => {
            ...
            if then_ty != otherwise_ty {
                return Err(crate::TuffError::TypeMismatch {
                    span: *span,
                    found: TypeEnv::type_name(otherwise_ty),
                    expected: TypeEnv::type_name(then_ty),
                    name: "if".into(),
                });
            }
            Ok(then_ty)
