...
void codegen_generate(CodeGen *gen)
{
	...
	while (decl)
	{
		if (decl->type == AST_INCLUDE)
		{
			emit(gen, "#include ");
			if (decl->data.include.is_system)
			{
				emit(gen, "<");
				emit(gen, decl->data.include.path);
				emit(gen, ">");
			}
			else
			{
				// For SafeC includes without .h, add .h suffix
				emit(gen, "\"");
				emit(gen, decl->data.include.path);
				// Check if it already has an extension
				const char *path = decl->data.include.path;
				const char *dot = strrchr(path, '.');
				if (!dot || (strcmp(dot, ".h") != 0 && strcmp(dot, ".c") != 0))
				{
					emit(gen, ".h");
				}
				emit(gen, "\"");
	...
	decl = gen->program->data.block.statements;
	while (decl)
	{
		switch (decl->type)
		{
...
// Generate a header file with struct definitions and function prototypes
void codegen_generate_header(CodeGen *gen, const char *guard_name)
{
	...
	// Emit struct definitions (non-generic only, as generics are monomorphized in .c)
	ASTNode *decl = gen->program->data.block.statements;
	while (decl)
	{
		switch (decl->type)
		{
		case AST_STRUCT_DEF:
			// For headers, emit non-generic struct definitions
			if (!decl->data.struct_def.type_params)
			{
				emit(gen, "struct ");
				emit(gen, decl->data.struct_def.name);
			...
			{
				// For generic structs, emit a comment noting it's a template
				emit(gen, "/* Generic struct ");
				emit(gen, decl->data.struct_def.name);
				emit(gen, " - instantiations are generated in .c file */\n\n");
			}
			break;

		case AST_FUNC_DEF:
			// Emit function prototypes (not the body)
			if (!decl->data.func_def.type_params)
			{
				emit_type(gen, decl->data.func_def.return_type, NULL);
				emit(gen, " ");
			...
			{
				// For generic functions, emit a comment
				emit(gen, "/* Generic function ");
				emit(gen, decl->data.func_def.name);
				emit(gen, " - instantiations are generated in .c file */\n");
			}
			break;
		...
		case AST_FUNC_DECL:
			// Already a declaration, emit as-is
			emit_type(gen, decl->data.func_def.return_type, NULL);
			emit(gen, " ");
			emit(gen, decl->data.func_def.name);
			emit(gen, "(");
