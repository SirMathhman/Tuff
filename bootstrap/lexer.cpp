...
Token Lexer::identifierOrKeyword() {
    ...
    static const std::unordered_map<std::string, TokenType> keywords = {
        {"expect", TokenType::EXPECT}, {"actual", TokenType::ACTUAL},
        ...
        {"type", TokenType::TYPE},
        {"I32", TokenType::I32}, {"Bool", TokenType::BOOL}, {"Void", TokenType::VOID}
    ...
    while (peek() != '\0') {
        ...
        if (isalpha(c) || c == '_') {
            ...
            switch (c) {
                ...
                case '=': 
                    if (match('=')) tokens.push_back({TokenType::EQUAL_EQUAL, "==", startLine, startCol});
                    else if (match('>')) tokens.push_back({TokenType::FAT_ARROW, "=>", startLine, startCol});
                    else tokens.push_back({TokenType::EQUALS, "=", startLine, startCol}); 
                    break;
                case '+': tokens.push_back({TokenType::PLUS, "+", startLine, startCol}); break;
                case '-': 
                    if (match('>')) tokens.push_back({TokenType::ARROW, "->", startLine, startCol});
