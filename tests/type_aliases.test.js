...
describe("type aliases", () => {
    ...
    it("uses type alias in multiple variable declarations", () => {
        expect(interpret("type Score = U8; let x : Score = 10; let y : Score = 20; x + y")).toBe(30);
    });
    it("throws on invalid assignment (type Temp = I32; let temp : Temp = 100; let value : Bool = temp)", () => {
        expect(() => interpret("type Temp = I32; let temp : Temp = 100; let value : Bool = temp;")).toThrow();
    });
    ...
    it("supports type alias with assignment to compatible value", () => {
        expect(interpret("type Count = I32; let x : Count = 10; let y : I32 = x; y")).toBe(10);
    });
});
