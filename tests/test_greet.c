...
static int test_execute_tuff_read_u8_add_u8_add_u8(void)
{
    int ret = execute_tuff("read<U8>() + read<U8>() + read<U8>()", "1 2 3");
    printf("    (execute_tuff returned %d)\n", ret);
    TEST("execute_tuff(\"read<U8>() + read<U8>() + read<U8>()\") with stdin \"1 2 3\" returns 6");
    ASSERT(ret == 6,
           "execute_tuff(\"read<U8>() + read<U8>() + read<U8>()\", \"1 2 3\") should return 6");
    return 0;
}

static int test_execute_tuff_read_u8_sub_u8(void)
{
    int ret = execute_tuff("read<U8>() - read<U8>()", "3 2");
    printf("    (execute_tuff returned %d)\n", ret);
    TEST("execute_tuff(\"read<U8>() - read<U8>()\") with stdin \"3 2\" returns 1");
    ASSERT(ret == 1,
           "execute_tuff(\"read<U8>() - read<U8>()\", \"3 2\") should return 1");
    return 0;
}

static int test_execute_tuff_let_x_u8(void)
{
    int ret = execute_tuff("let x : U8 = read<U8>(); x", "100");
    printf("    (execute_tuff returned %d)\n", ret);
    TEST("execute_tuff(\"let x : U8 = read<U8>(); x\") with stdin \"100\" returns 100");
    ASSERT(ret == 100,
           "execute_tuff(\"let x : U8 = read<U8>(); x\", \"100\") should return 100");
    return 0;
}
