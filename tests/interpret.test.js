...
describe("interpret - parentheses, division & modulus", () => {
    ...
    it("modulus and modulo-by-zero handling", () => {
        ...
        if (isErr(m2))
            expect(m2.error).toBe("Division by zero");
        const m3 = interpret("10 + 5 % 3");
        expect(isOk(m3)).toBe(true);
        if (isOk(m3))
            expect(m3.value).toBe(12);
    });
...
describe("interpret - identifiers", () => {
    it("returns Err on undefined identifier", () => {
        ...
        expect(isErr(r)).toBe(true);
        if (isErr(r))
            expect(r.error).toBe("Undefined variable");
    });
...
describe("interpret - let & assignment", () => {
    ...
    it("errors when assigning to immutable or undefined variables", () => {
        ...
        expect(isErr(r)).toBe(true);
        if (isErr(r))
            expect(r.error).toBe("Cannot assign to immutable variable");
        const r2 = interpret("y = 1;");
        expect(isErr(r2)).toBe(true);
        if (isErr(r2))
            expect(r2.error).toBe("Undefined variable");
    });
});
describe("interpret - compound assignment", () => {
    ...
    it("errors on compound assignment to immutable or uninitialized or undefined variables", () => {
        ...
        expect(isErr(r2)).toBe(true);
        if (isErr(r2))
            expect(r2.error).toBe("Uninitialized variable");
        const r3 = interpret("y += 1;");
        expect(isErr(r3)).toBe(true);
        if (isErr(r3))
            expect(r3.error).toBe("Undefined variable");
        const r4 = interpret("let mut z : I32 = 1; z /= 0;");
        expect(isErr(r4)).toBe(true);
        if (isErr(r4))
            expect(r4.error).toBe("Division by zero");
    });
});
...
describe("interpret - let & assignment (misc)", () => {
    ...
    it("errors when reading uninitialized variable", () => {
        ...
        expect(isErr(r)).toBe(true);
        if (isErr(r))
            expect(r.error).toBe("Uninitialized variable");
    });
...
describe("interpret - if expressions (statement branches)", () => {
    ...
    it("supports else-if chains (first branch)", () => {
        const r = interpret("let x : I32; if (true) x = 3; else if (true) x = 5; else x = 4; x");
        expect(isOk(r)).toBe(true);
        if (isOk(r))
            expect(r.value).toBe(3);
    });
    it("supports else-if chains (second branch)", () => {
        const r = interpret("let x : I32; if (false) x = 3; else if (true) x = 5; else x = 4; x");
        expect(isOk(r)).toBe(true);
        if (isOk(r))
            expect(r.value).toBe(5);
    });
    it("supports else-if chains (else branch)", () => {
        const r = interpret("let x : I32; if (false) x = 3; else if (false) x = 5; else x = 4; x");
        expect(isOk(r)).toBe(true);
        if (isOk(r))
            expect(r.value).toBe(4);
    });
...
describe("interpret - complex block operations", () => {
    ...
    it("block in match condition", () => {
        const r = interpret("let x : I32; x = match ({ 1 }) { case 1 => 42; default => 0; }; x");
        expect(isOk(r)).toBe(true);
        if (isOk(r))
