...
describe("interpret control flow and comparisons", () => {
    ...
    it("handles nested if expressions", () => {
        expect(interpret("let x = if (true) { if (false) { 1 } else { 2 } } else { 3 }; x")).toBe(2);
    });
    it("handles else if", () => {
        expect(interpret("let x = if (false) { 1 } else if (true) { 2 } else { 3 }; x")).toBe(2);
    });
    it("handles complex conditional with comparison", () => {
        expect(interpret("let x = if ({let z = 10; z} < {let a = 140; a}) { let y = 200; y } else 400; x")).toBe(200);
    });
...
describe("interpret loops", () => {
    it("handles while loops", () => {
      ...
      total
    `)).toBe(10);
        expect(interpret("let mut x = 0; let y = while (x < 3) { x += 1; x }; y")).toBe(3);
    });
    it("handles do-while loops", () => {
        ...
        expect(interpret("let mut x = 0; let y = do { x += 1; x } while (x < 3); y")).toBe(3);
    });
    it("handles match expressions", () => {
        expect(interpret("let x = match (100) { case 100 => 2; case _ => 1; }; x")).toBe(2);
        expect(interpret("let x = match (50) { case 100 => 2; case _ => 1; }; x")).toBe(1);
        expect(interpret(`
      let y = match (1) {
        case 0 => 10;
        case 1 => 20;
      ...
      y
    `)).toBe(20);
        expect(interpret("match (10 + 10) { case 20 => { 1 + 1 }; case _ => 0; }")).toBe(2);
    });
});
