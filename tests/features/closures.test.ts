...
describe('closures - basics', (): void => {
	beforeEach((): void => {
		clearFunctionRegistry();
	});

	it('captures mutable variable and modifies it', (): void => {
		assertInterpretAndCompileValid('let mut x = 0; fn add() => { x += 1; x }; add(); add(); x', 2);
	});

	it('closure reads captured variable', (): void => {
		assertInterpretAndCompileValid('let y = 10; fn get() => y; get()', 10);
	});
	...
	it('closure sees variable updates', (): void => {
		assertInterpretAndCompileValid('let mut x = 1; x = 5; fn get() => x; get()', 5);
	});
});

describe('closures - nested functions', (): void => {
	beforeEach((): void => {
		clearFunctionRegistry();
	});
	...
	it('nested function captures outer function parameter', (): void => {
		assertInterpretAndCompileValid(
			'fn pass(value : I32) => { fn get() => value; get() } pass(100)',
			100,
		);
	});
...
describe('closures - returns closure', (): void => {
	beforeEach((): void => {
		clearFunctionRegistry();
	});
...
describe('closures - compiler read<T>() tests', (): void => {
	beforeEach((): void => {
		clearFunctionRegistry();
	});

	it('closure captures runtime input', (): void => {
		assertCompileValid('let y = read<I32>(); fn get() => y; get()', '42', 42);
	});

	it('closure modifies runtime input', (): void => {
		assertCompileValid(
			'let mut x = read<I32>(); fn add() => { x += 1; x }; add(); add(); x',
			'10',
			12,
		);
	});
