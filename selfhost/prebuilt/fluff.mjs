...
import {
  fluff_project_with_reader,
  fluff_files_with_reader,
} from "./tuffc_lib.mjs";
import {
  set_fluff_options,
  set_fluff_debug_options,
  set_fluff_debug_scopes,
  set_fluff_complexity_options,
  set_fluff_file_size_options,
  set_fluff_max_params_options,
  set_fluff_single_char_identifiers_options,
  set_fluff_missing_docs_options,
  set_fluff_clone_detection_options,
  set_fluff_clone_parameterized_options,
  set_fluff_simplify_negation_options,
} from "./analyzer.mjs";
import { load_fluff_config } from "./build_config.mjs";
import {
  set_diagnostics_format,
  has_project_errors,
  reset_project_errors,
  get_project_error_count,
  get_project_warning_count,
} from "./util/diagnostics.mjs";
...
export function print_usage() {
  ...
  println(
    "  --debug                       Print all debug output (very noisy)"
  );
...
export function run(argv) {
  ...
  set_fluff_clone_detection_options(
    cfg.cloneDetection,
    cfg.cloneMinTokens,
    cfg.cloneMinOccurrences
  );
  set_fluff_clone_parameterized_options(cfg.cloneParameterized);
  set_fluff_simplify_negation_options(cfg.simplifyNegation);
  vec_len(inPaths) == 1
    ? (() => {
