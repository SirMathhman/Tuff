...
let start = 0;
let stop = stringLen(s);
while (start < stop && is_ascii_ws(stringCharCodeAt(s, start))) {
start = start + 1;
}
while (stop > start && is_ascii_ws(stringCharCodeAt(s, stop - 1))) {
stop = stop - 1;
}
return stringSlice(s, start, stop);
}
