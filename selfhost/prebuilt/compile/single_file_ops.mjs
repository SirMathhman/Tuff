...
import {
  ...
  is_fn_decl_start,
} from "../parsing/decls.mjs";
import { span, decl_let, decl_let_typed } from "../ast.mjs";
import {
  analyze_program_with_fns,
  check_file_size,
  check_clones,
} from "../analyzer.mjs";
import {
  decls_needs_vec_rt,
  emit_runtime_vec_imports_js,
  set_current_file_path,
} from "../emit/emit_helpers.mjs";
  ...
  while (true) {
    ...
    if (starts_with_at(src, j, "let")) {
      ...
      if (tyAnn == "") {
        vec_push(
          decls,
          decl_let(span(start, i), mutOpt.ok, name.text, expr.expr)
        );
      } else {
        vec_push(
          decls,
          decl_let_typed(span(start, i), mutOpt.ok, name.text, tyAnn, expr.expr)
        );
      }
      continue;
    }
...
export function compile_tiny2(src, requireMain, exportAll, filePath) {
  return compile_tiny2_with_imported_fns(
    src,
    requireMain,
    exportAll,
    filePath,
...
export function compile_tiny2_with_imported_fns(
  src,
  requireMain,
  exportAll,
  filePath,
...
export function lint_tiny2_with_imported_fns(
  src,
  requireMain,
  exportAll,
  importedFns
...
export function lint_tiny2_collect_with_imported_fns(
  src,
  requireMain,
  exportAll,
  importedFns
