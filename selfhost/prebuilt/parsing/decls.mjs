...
export function parse_extern_decl_ast(src, i) {
  ...
  if (starts_with_at(src, j1, "from")) {
    ...
    return ParsedDeclAst(
      decl_extern_from(span(start, k), mod.text, names.names),
      k
    );
  }
  if (starts_with_at(src, j1, "type")) {
    ...
    return ParsedDeclAst(
      decl_extern_type(span(start, k), isOut, name.text, typeParams),
      k
    );
  }
...
export function parse_type_union_decl_ast(src, i, exportAll) {
  ...
  while (true) {
    ...
    if (t1 < stringLen(src) && stringCharCodeAt(src, t1) == 60) {
      ...
      vec_push(
        variants,
        type_union_variant_typed(span(vStart, k), v.text, payloadTyAnns)
      );
    } else {
  ...
  return ParsedDeclAst(
    decl_type_union(span(start, k), name.text, typeParams, variants),
    k
  );
}
