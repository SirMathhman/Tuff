...
## Workflow
- **Test-first**: Always add the test case in `src/main.rs` *before* implementing any new feature or fixing a bug. The hook will block commits that don't pass with 100% coverage, so write the failing test first, then implement just enough code to make it pass and cover all branches.
