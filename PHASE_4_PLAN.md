...
**Current Status**: Analyzer implements no-shadowing, mutability checks, basic type inference/validation (incl. Bool conditions), **let type annotations**, and **basic struct + function signature validation**.
...
5. **Type Inference Stubs**
   ...
   6. **Type Annotations Enforcement (Phase 4a)**

      - `let x: T = expr` validates inferred type against annotated `T`
      - Binding type tags record the annotation when present (enables downstream checks)
      - Tests: `selfhost_analyzer_phase4a_structs.test.ts`

   7. **Basic Struct Registry + Validation (Phase 4a)**

      - Predeclares `struct` definitions (name, field order, optional field type annotations)
      - Validates positional struct literals for arity and (when provided) field type annotations
      - Validates field access (`p.x`) when base type is a known struct
      - Tests: `selfhost_analyzer_phase4a_structs.test.ts`

   8. **Basic Function Signature Tracking + Call Validation (Phase 4b, partial)**
      - Predeclares `fn`/`class fn` arity + optional param/return annotations
      - Validates call arity, and validates argument types when annotations exist
      - Validates function return/yield types when return annotation exists
...
#### Gap: Type Annotations Are String-Based (Phase 4a)

- **Issue**: Analyzer currently treats type annotations as normalized strings (not resolved `TypeRef`s)
- **Impact**: This intentionally avoids false negatives for generics, aliases, and unresolved names, but limits precision
- **Next**:
  - Move toward resolved types (e.g. `TypeRef` → resolved type) once generics/aliases are implemented
  - Expand coverage to additional expression forms as they become modeled

#### Gap: Struct Validation Is Positional-Only (Phase 4a)

- **Issue**: Struct literals are positional (not named fields), and completeness/defaults aren't modeled
- **Impact**: Can't express or validate missing fields by name; no defaulting semantics
- **Next**:
  - If/when named field literals are added, validate field presence and detect duplicates
  - Add richer struct literal diagnostics (expected field list)

#### Gap: Function Signatures Are Partial (Phase 4b)

- **Issue**: Function signatures are tracked, but type validation is only enforced when annotations exist
- **Impact**: Unannotated functions/calls may remain `Unknown` and skip validation to avoid false positives
- **Next**:
  - Improve inference for common expressions and propagate inferred return types
  - Extend call checking to more callee forms (e.g. paths) and richer type forms
...
1. **Parse and Store Type Annotations** ✅
   ...
   - Stored on `SLet`/`DLet` nodes as strings (from parser)
   - Analyzer validates annotated type against inferred type
   - **Tests**: `src/test/ts/selfhost_analyzer_phase4a_structs.test.ts`
...
**Tests Added**: `src/test/ts/selfhost_analyzer_phase4a_structs.test.ts`
...
- [selfhost*analyzer*\*.test.ts](src/test/ts/) — Existing test suite
