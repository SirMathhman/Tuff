...
describe("evaluate", () => {
  ...
  it('evaluate("let x = { let mut y = 100; y = 1; }; x") => Error', () => {
    expect(() => evaluate("let x = { let mut y = 100; y = 1; }; x")).toThrow();
  });
  ...
  it('evaluate("let x = if (false) 1 else 2; x") => 2', () => {
    expect(evaluate("let x = if (false) 1 else 2; x")).toBe(2);
  });
