...
export function expectValid(tuffSource : string, expectedExitCode : number, args : string[] = []) {
    ...
    new Function("process", "args", compiledJS)(process, ["mock_program_name",  ...args]);
...
export function expectInvalid(tuffSource : string) {
    try {
        const compiledTS =         compileTuffToTypeScript(tuffSource);
        throw new Error("Expected test to fail but compiled to: '" + compiledTS + "'")
    } catch(e) {
    }
}