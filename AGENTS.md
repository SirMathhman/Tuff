...
- **Type system**: Analyzer resolves all types onto AST nodes. Un-suffixed numbers stay `dynamic()` until `typecheck` site (I32 default). Context propagation: dynamic operands inherit from concrete siblings in binary ops. Union types (`type Alias = T1 | T2`) are resolved by `resolveTypeNode` recursively resolving each variant.
