...
- **Result monad, no exceptions.** All fallible functions return `Result<T, EvalError>` (`src/core/errors.ts`). `EvalError` is a discriminated union on `kind`, one named `interface` per variant, each carrying a zero-based character `position` and a doc comment stating the fix. New error kinds get their own variant — never shoehorn into an existing one.
- **Total typecheck, total evaluator.** The static pass (`evaluator/typecheck.ts` + `checkExpressions.ts` + `checkAssignments.ts`) must detect every error the evaluator can produce; the evaluator resolves values and never throws. Defensive evaluator branches return structured `EvalError`s with accurate payloads, never placeholder values.
